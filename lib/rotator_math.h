...
// Float version used for speed conversion (deg/s -> steps/s)
inline float deg2step_f(float deg, float ratio, uint8_t microsteps) {
    return (ratio * (float)SPR * (float)microsteps * deg) / 360.0f;
}
